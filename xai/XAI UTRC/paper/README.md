...
- Table I on p2; Fig. 1 p3; Figs. 2-3 p4; Fig. 4 p5
...
Upload the whole `XAI UTRC/` directory, not just `paper/`. Set the main document
to `paper/main.tex` and the compiler to **pdfLaTeX** (Menu -> Compiler).
Overleaf runs bibtex automatically.

## Requirements

`IEEEtran.cls` plus `graphicx`, `amsmath`, `booktabs`, `array`, `xcolor`,
